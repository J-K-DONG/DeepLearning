...
def train(opt, Gs, Zs, reals, NoiseAmp):
    ...
    while scale_num < opt.stop_scale + 1:
        ...
        plt.imsave('%s/real_scale.png' %  (opt.outf), functions.convert_image_np(reals[scale_num]), vmin=0, vmax=1)  # 保存当前层的尺度的图像

        D_curr, G_curr = init_models(opt)
        if (nfc_prev == opt.nfc):  # 用现有模型训练
...
def train_single_scale(netD, netG, reals, Gs, Zs, in_s, NoiseAmp, opt, centers=None):
    ...
    if opt.mode == 'animation_train':
        ...
        opt.nzy = real.shape[3] + (opt.ker_size - 1) * (opt.num_layer)
        pad_noise = 0
    m_noise = nn.ZeroPad2d(int(pad_noise))  # 每个边 填充pad_noise个0
    m_image = nn.ZeroPad2d(int(pad_image))

    alpha = opt.alpha

    fixed_noise = functions.generate_noise([opt.nc_z, opt.nzx, opt.nzy], device = opt.device)
    z_opt = torch.full(fixed_noise.shape, 0, device = opt.device)  # 置零
    z_opt = m_noise(z_opt)  # 填充
