...
def np2torch(x, opt):
    if opt.nc_im == 3:  # 图像3通道
        x = x[:, :, :, None]  # 长， 宽， 通道数， None
        x = x.transpose((3, 2, 0, 1))/255  # none， 通道数， 长， 宽
    else:
        x = color.rgb2gray(x)
        x = x[:, :, None, None]
...
def adjust_scales2image(real_, opt):
    ...
    opt.num_scales = math.ceil((math.log(math.pow(opt.min_size / (min(real_.shape[2], real_.shape[3])), 1), opt.scale_factor_init))) + 1  # 计算层数
...
def generate_dir2save(opt):
    ...
    if (opt.mode == 'train') | (opt.mode == 'SR_train'):
        dir2save = 'TrainedModels/%s/scale_factor=%f_alpha=%d' % (opt.input_name[:-4], opt.scale_factor_init, opt.alpha)
